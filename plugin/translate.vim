...
if !hasmapto('<Plug>TRV_TransVimVisual')
    vmap <silent> <unique> <F4> <Plug>TRV_TransVimVisual
endif

if !hasmapto('<Plug>TRV_TransVimNormal')
    nmap <silent> <unique> <F4> <Plug>TRV_TransVimNormal
endif

if !hasmapto('<Plug>TRV_TransVimAsk')
    map <silent> <unique> [[ <Plug>TRV_TransVimAsk
endif
...
nmap \<F4> :call ToggleTranslate()<cr>
let g:translate_toggle_enable = 1 
fun! ToggleTranslate()
    if g:translate_toggle_enable
        autocmd CursorHold <buffer> call <SID>TRV_TransVimWord(expand("<cword>"))
        " autocmd CursorHold * call <SID>TRV_TransVimWord(expand("<cword>"))
        set ut=100
        echo 'show translation automatically'
    else
        autocmd! CursorHold <buffer>
        set ut=4000
        echo 'shutdown auto translation'
    endif
...
function s:TransVim(str)
    ...
    if s:strng == ""                     " is there a string to search for?
        ...
        if g:translate_toggle_enable " means not toggled on yet
...
"**                                                                         **
"*****************************************************************************
let s:not_exist_translation_window = 1
